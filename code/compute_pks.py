...
def main():
    ...
    compute_pks_loop(idxs_sam, ndens_target, args_power, overwrite=False, n_threads=n_threads)
...
def compute_pks_loop(idxs_sam, ndens_target, args_power, overwrite=False, n_threads=2):
    ...
    if n_threads>1:
        ...
        outputs = pool.map(partial(compute_pk, ndens_target=ndens_target, 
                                   args_power=args_power, overwrite=overwrite), idxs_sam)
        print("Done!")
    else:
        outputs = []
        for idx_sam in idxs_sam:
            output = compute_pk(idx_sam, ndens_target=ndens_target,
...
def compute_pk(idx_sam, ndens_target=None, args_power=None, overwrite=False):

    assert ndens_target is not None and args_power is not None, "Must pass ndens_target and args_power!"
